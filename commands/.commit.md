...
Look at recent commits for initials like `AB:`, `CD:` at the start.
...
```
AB: fix(auth): correct token expiry check
```
Single line only.

**For SIGNIFICANT changes (new feature, major refactor, lots of logic):**
```
AB: feat(auth): add JWT authentication
...
Example:
```
AB: feat(payment): add Stripe integration
